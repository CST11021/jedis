 ...
 */
public class Jedis extends BinaryJedis implements BasicCommands, JedisCommands, MultiKeyCommands, SentinelCommands, ClusterCommands, AdvancedJedisCommands, ScriptingCommands, ModuleCommands {
    ...
    /**
     * 将字符串值设置为键的值。字符串不能超过1073741824字节（1GB）, 时间复杂度：O(1)
     *
     * @param key
     * @param value
    ...
    /**
     * 将字符串值设置为键的值。字符串不能超过1073741824字节（1GB）
     *
     * @param key
     * @param value
     * @param params    NX|XX：
    ...
    /**
     * GETSET命令是原子操作，设置此值并返回旧值命令，将key设置为字符串值并返回存储在key处的旧值。字符串不能超过1073741824字节（1 GB）。
     * Time complexity: O(1)
     *
     * @param key
     * @param value
    ...
    /**
     * 获取指定键的值。如果键不存在，则返回null. 如果存储在key处的值不是字符串，则返回错误，因为GET仅能处理字符串值.
     * Time complexity: O(1)
     *
     * @param key
     * @return Bulk reply
    ...
    /**
     * 测试指定的密钥是否存在。如果密钥存在，则命令返回true，否则返回false。请注意，即使将键设置为空字符串也将返回true。
     * Time complexity: O(1)
     *
     * @param key
    ...
    /**
     * 从当前选择的数据库中返回一个随机选择的key
     * Time complexity: O(1)
     *
     * @return 单行回复，特别是随机选择的键或空字符串表示数据库为空
     */
    @Override
    ...
    /**
     * 以原子方式将密钥oldkey重命名为newkey。如果源名称和目标名称相同，则返回错误。如果newkey已经存在，它将被覆盖。
     * Time complexity: O(1)
     *
     * @param oldkey
     * @param newkey
    ...
    /**
     * 将oldkey重命名为newkey，但是如果目标键newkey已经存在，则失败。
     * Time complexity: O(1)
     *
     * @param oldkey
     * @param newkey
     * @return 整数回复，特别是：如果密钥已重命名，则为1；如果目标密钥已存在，则为0。
     */
    @Override
    ...
    /**
     * 将指定的键从当前所选的DB移动到指定的目标DB. 请注意，只有成功移动了key，此命令才会返回1；如果目标密钥已经存在，或者根本找不到源key，那么此命令将返回0，因此可以将MOVE用作锁定原语。
     *
     * @param key
     * @param dbIndex
    ...
    /**
     * 该命令完全等效于以下命令组：
     * {@link #set(String, String) SET} + {@link #expire(String, int) EXPIRE}. 该操作是原子的。
     * <p>
     * Time complexity: O(1)
     *
     * @param key
    ...
    /**
     * 将各个键设置为各个值，MSET命令会将旧值替换为新值，而{@link #msetnx(String...)} 将根本不执行任何操作，即使仅存在一个键也是如此。
     * 由于这种语义，可以使用MSETNX来设置表示唯一逻辑对象的不同字段的不同键，以确保所有字段都设置或根本不设置。
     * <p>
     * MSET和MSETNX都是原子操作。这意味着，例如，如果key A和B被修改，则与Redis对话的另一个客户端可以一次看到对A和B的更改，或者根本看不到任何更改。
     *
     * @param keysvalues
     * @return 状态代码回复基本上返回OK，因为MSET不会失败
     * @see #msetnx(String...)
     */
    @Override
    ...
    /**
     * INCRBYFLOAT命令仅限于双精度浮点值。
     * <p>
     * 注意：这实际上是一个字符串操作，也就是说，在Redis中没有“double”类型。
     * 只需将存储在key处的字符串解析为基本的双精度浮点值，递增，然后转换回字符串。没有DECRYBYFLOAT，但是提供负值将可以正常工作。
     * <p>
     * Time complexity: O(1)
     *
     * @param key
    ...
    /**
     * 将key存储的数字增加1，如果键不存在或包含错误类型的值，请在执行增量操作之前将value设置为“ 0”，INCR命令限于64位有符号整数，如果value存储不是数值，则会报错
     *
     * 注意：这实际上是一个字符串操作，也就是说，在Redis中没有“integer”类型，只需将存储在key对应的字符串解析为以10为基数的64位有符号整数，然后递增，然后再转换回字符串即可。
     *
     * Time complexity: O(1)
     *
     * @param key
    ...
    /**
     * 返回从偏移量开始到偏移量结束的字符串的子集（包括偏移量），可以使用负偏移量来提供从字符串末尾开始的偏移量。因此-1表示最后一个字符，-2表示倒数第二个，依此类推。
     *
     * 该函数可以处理超出范围的请求，而不会引发错误，而只是将结果范围限制为字符串的实际长度。
     *
    ...
    /**
     * 将指定的哈希字段设置为指定的值，如果key不存在，则会创建一个包含哈希的新key。
     * Time complexity: O(1)
     *
     * @param key
     * @param field
    ...
    /**
     * 如果key包含哈希，则检索与指定字段关联的值，如果找不到该字段或该键不存在，则返回一个特殊的“ nil”值。
     * Time complexity: O(1)
     *
     * @param key
     * @param field
     ...
     * @param field
     * @param value
     * @return 如果该字段已经存在，则返回0，否则，如果创建了一个新字段，则返回1
     * returned.
     */
    @Override
    ...
    /**
     * 将各个字段设置为各个值，HMSET用新值替换旧值，如果密钥不存在，则会创建一个包含哈希的新key。
     * Time complexity: O(N) (with N being the number of fields)
     *
     * @param key
     * @param hash
     * @return 如果哈希为空，则返回OK或Exception
     */
    @Override
    ...
    /**
     * 检索与指定字段关联的值，如果某些指定的字段不存在，则返回nil值，不存在的key被视为空哈希。
     * Time complexity: O(N) (with N being the number of fields)
     *
     * @param key
     * @param fields
     * @return 多批量答复专门按照请求的相同顺序列出与指定字段关联的所有值
     */
    @Override
    ...
    /**
     * 按值递增存储在key处的哈希中字段中存储的数字。
     * If key does not exist, a new key holding a hash is created. If field does not exist or holds a string, the value is set to 0
    ...
    /**
     * 返回哈希中的项目数 Time complexity: O(1)
     *
     * @param key
     * @return key中存储的哈希中包含的条目（字段）数。如果指定的key不存在，则假定为空哈希，则返回0
     */
    @Override
    ...
    /**
     * 修剪现有列表，使其仅包含指定范围的指定元素，开始和结束是从零开始的索引，0是列表的第一个元素（列表头），1是下一个元素，依此类推。
    ...
    @Override
    public String migrate(final String host, final int port, final int destinationDB, final int timeout, final MigrateParams params, final String... keys) {
        checkIsInMultiOrPipeline();
